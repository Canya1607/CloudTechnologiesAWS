...
resource "aws_subnet" "rds_public_us_east_2a" {
...
resource "aws_subnet" "rds_public_us_east_2b" {
  vpc_id     = aws_vpc.my_vpc.id
...
resource "aws_route_table_association" "my_vpc_us_east_2a_public" {
    subnet_id = aws_subnet.rds_public_us_east_2a.id
    route_table_id = aws_route_table.my_vpc_public.id
}

resource "aws_route_table_association" "my_vpc_us_east_2b_public" {
    subnet_id = aws_subnet.rds_public_us_east_2b.id
    route_table_id = aws_route_table.my_vpc_public.id
}
...
resource "aws_db_subnet_group" "rds-private-subnet" {
  ...
  subnet_ids = ["${aws_subnet.rds_public_us_east_2a.id}", "${aws_subnet.rds_public_us_east_2b.id}"]
}
